...
class MoICV_Qwen_Wrapper(nn.Module):
    ...
    def _setup_hooks(self) -> None:
        ...
        def attn_pre_hook(module: nn.Module, inputs: tuple) -> tuple:
            """
            在 Self-Attention 前注入 v_attn。
            典型 inputs 格式（以 Qwen2 为例）：
                (hidden_states, attention_mask, position_ids, ... )
            我们只修改第 0 个元素 hidden_states。
            """
            ...
            # hidden_states: [B, S, D]；current_v_attn: [B, D]
            ...
            # 自动对齐 device / dtype
            if v_attn.device != hidden_states.device:
                v_attn = v_attn.to(hidden_states.device)
            if v_attn.dtype != hidden_states.dtype:
                v_attn = v_attn.to(hidden_states.dtype)

            # 在 seq_len 维度上广播： [B, 1, D]
            v_attn_expanded = v_attn.unsqueeze(1)
            if v_attn_expanded.size(-1) != hidden_states.size(-1):
                # 维度不匹配则跳过注入，但不报错，以防中途改变 hidden_size
                return inputs

            hidden_states = hidden_states + self.gamma_attn * v_attn_expanded
            # 重新构造新的 inputs 元组
            new_inputs = (hidden_states,) + tuple(inputs[1:])
            return new_inputs

        handle_attn = attn_module_for_hook.register_forward_pre_hook(attn_pre_hook, with_kwargs=False)
        ...
        def ffn_pre_hook(module: nn.Module, inputs: tuple) -> tuple:
            """
            在 MLP 前注入 v_ffn。
            典型 inputs 格式：
                (hidden_states, ) 或 (hidden_states, *extra)
            """
            ...
            if v_ffn.device != hidden_states.device:
                v_ffn = v_ffn.to(hidden_states.device)
            if v_ffn.dtype != hidden_states.dtype:
                v_ffn = v_ffn.to(hidden_states.dtype)

            # [B, D] -> [B, 1, D] 广播到整个序列
            v_ffn_expanded = v_ffn.unsqueeze(1)
            if v_ffn_expanded.size(-1) != hidden_states.size(-1):
                return inputs

            hidden_states = hidden_states + self.gamma_ffn * v_ffn_expanded
            new_inputs = (hidden_states,) + tuple(inputs[1:])
            return new_inputs

        handle_ffn = ffn_module_for_hook.register_forward_pre_hook(ffn_pre_hook, with_kwargs=False)
